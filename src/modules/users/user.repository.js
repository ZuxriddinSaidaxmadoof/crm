...
export class UserRepository extends Postgres {
  ...
  async findOneByPhone(id) {
    return await this.fetch("select * from users where email = $1", id);
  }
  async create(dto) {
    return await this.fetch(`
    INSERT INTO users(first_name, last_name, email,password, role)
    VALUES ($1,$2,$3,$4,$5) RETURNING *;
    `, dto.first_name, dto.last_name, dto.email, dto.password, dto.role);
  }
  async update(dto, id) {
    return await this.fetch(`UPDATE users SET first_name = $1, last_name = $2, email = $3, password =$4 WHERE id = $5 RETURNING *;`, dto.first_name, dto.last_name,dto.email,dto.password, id);
  }
  async delete(id) {
